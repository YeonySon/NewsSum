...
public class MyPageService {
    ...
    public List<NewsResponseDto> selectByMyNews(Integer userId, Pageable pageable) {
        ...
        if (totalPages >= currentPage) {
            ...
            Page<News> lastPage = newsRepository.selectAllMyReadNews(userId,
        ...
        List<NewsResponseDto> result = newsService.makeNewsResponseDto(newsList.getContent(), resultList, userId,
                totalPages);
    ...
    public List<NewsResponseDto> selectMyScrapByCategoryId(Integer userId, Integer categoryId, Pageable pageable) {
        ...
        if (currentPage >= totalPages) {
            ...
            // 전체 뉴스기사
            if (categoryId == 0) {
                lastPage = newsRepository.selectAllMyScrapNews(userId, PageRequest.of(totalPages, pageable.getPageSize()));
            } else {
                // 카테고리별 뉴스기사
                lastPage = newsRepository.selectMyScrapNewsByCategory(userId, categoryId, PageRequest.of(totalPages, pageable.getPageSize()));
            }
            newsList = lastPage;
        }
    ...
    public List<NewsResponseDto> selectScrapNewsSortByOption(Integer userId, Integer categoryId, Integer optionId,
        ...
        if (currentPage >= totalPages) {
            ...
            if (categoryId == 0) {
                ...
                // 인기도 순
                if (optionId == 1) {
                    lastPage = newsRepository.selectAllScrapPopular(userId, PageRequest.of(totalPages, pageable.getPageSize()));
                }
                // 최신 순
                else {
                    lastPage = newsRepository.selectAllMyScrapNews(userId, PageRequest.of(totalPages, pageable.getPageSize()));
            ...
            // 해당 분야별
                ...
                // 인기도순
                if (optionId == 1) {
                    lastPage = newsRepository.selectScrapCategoryByOption(userId, categoryId, PageRequest.of(totalPages, pageable.getPageSize()));
                }
                // 최신순
                else {
                    lastPage = newsRepository.selectMyScrapNewsByCategory(userId, categoryId, PageRequest.of(totalPages, pageable.getPageSize()));
    ...
    public List<JobCntDto> selectJobsByReadNews(Integer userId) {
        ...
        List<News> readNews = newsRepository.selectAllReadNews(userId);
