...
public interface NewsRepository extends JpaRepository<News, Integer> {
    ...
    // 카테고리별 리스트 조회
    @Query("select n from News n where n.cgId.categoryId =:categoryId " +
            "and n.image != null " +
    ...
    // 뉴스 전체 인기도순으로 조회
    @Query("select n from News n where n.image != null order by (n.viewCnt + n.totalLike + n.totalScrap) desc, n.postedDate desc")
    ...
    @Query("select n from News n where n.head like %:keyword% and n.image !=null order by n.postedDate desc")
    ...
    @Query("select n from News n, Scrap s where s.user.userId =:userId and s.contentId = n.newsId " +
            "and n.cgId.categoryId =:categoryId and n.image != null " +
            "order by (n.viewCnt + n.totalLike + n.totalScrap) desc, s.createdAt desc")
